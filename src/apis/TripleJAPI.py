...
class TripleJ():
    def __init__(self,url,limit,logger):
        # url to query
        self.url = url 
        # limit to base how many queries each interval
        self.limit = limit
        self.url = self.url + '&limit=' + limit
        # pass in the self.logger.so we can track if things go wrong
        self.logger= logger
    ...
    def get_songs(self):
        ...
        self.logger.info('Getting songs from Triple J...')
        songs = []
        ...
        for track in tracks['items']:
            if track['service_id'] == 'triplej':
                ...
                self.logger.info('Found track \'%s\' by \'%s\'' % (title, artist))
